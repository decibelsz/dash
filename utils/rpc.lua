-- credits: https://github.com/luizgustavof/fivem-rpc
local server = IsDuplicityVersion()
local emitNet = server and TriggerClientEvent or TriggerServerEvent
local subscribe = server and RegisterServerEvent or RegisterNetEvent
...
function rpc:call(syncOptions, name, ...)
    ...
    local params = {...}
    local args = server and {params[1], uuid, name, table.unpack(params, 2)} or {uuid, name, table.unpack(params)}
...
function rpc:listener()
    subscribe(
        ('connection-%s:request'):format(self.name),
        function(uuid, name, ...)
            local source = source
            ...
            local resume = server and {source, uuid, response} or {uuid, response}
    ...
    subscribe(
        ('connection-%s:response'):format(self.name),
